...
class OpenLibraryClient(BaseApiClient):
    ...
    def _extract_book_data(self, doc: dict) -> dict:
        ...
        if publisher := doc.get("publisher"):
            result["publisher"] = publisher[0] if isinstance(publisher, list) and publisher else publisher
        
        # Language
        if language := doc.get("language"):
            result["language"] = language[0] if isinstance(language, list) and language else language
